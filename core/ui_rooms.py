...
1. 📡 Máy Quét Đọc Vị Thế Cuộc (AI Macro Radar)
2. 🎯 Phân Rã Thực Chiến & Nhật Ký Quyết Định (9 Lenses & Decision Journal)
"""
...
def render_macro_radar_room(active_api_key: str | None = None, is_embedded: bool = False):
    """Render toàn diện phòng chức năng Máy Quét Đọc Vị Thế Cuộc & 8 Mật Mã Giới Elite."""
    if not is_embedded:
    ...
    m_tab1, m_tab2, m_tab3, m_tab4 = st.tabs([
        ...
        f"📚 Kho Lưu Trữ Quét ({len(load_macro_scans())})",
    ])
    ...
    with m_tab2:
        st.markdown("#### ⏳ Trục Tiến Hóa 5 Kỷ Nguyên & Định Luật Chuyển Pha")
        st.caption("Quy luật bất biến: Khi một kỷ nguyên chạm trần năng lượng và sức chứa sinh thái, nó buộc phải chuyển pha hoặc tự sụp đổ.")
        for era in CIVILIZATIONAL_ERAS:
            with st.expander(f"{era['icon']} Kỷ Nguyên {era['era_number']}: {era['name']} ({era['timeframe']})"):
                col_e1, col_e2 = st.columns([3, 2])
                with col_e1:
                    st.markdown(f"**Nguồn lực cốt lõi:** {era['core_resource']}")
                    st.markdown(f"**Công nghệ & Năng lượng:** {era['energy_tech']}")
                    st.markdown(f"**Ràng buộc then chốt:** {era['constraint']}")
                    st.markdown(f"**Bản chất chuyển pha:** {era['turning_point']}")
                with col_e2:
                    st.error(f"📉 **Hàng hóa phổ biến (Trượt giá về 0):**  \n{era['commoditized']}")
                    st.success(f"💎 **Tài sản khan hiếm mới lên ngôi:**  \n{era['new_scarce_asset']}")
                    st.info(f"👑 **Đòn bẩy của Giới Tinh Hoa:**  \n{era['elite_leverage']}")
                st.markdown(f"💡 *{era['deep_dive']}*")

    with m_tab3:
        st.markdown("#### 👁️ 8 Mật Mã Vận Hành Ngầm Của Giới Elite (The Hidden Elite Playbook)")
        st.caption("Các cơ chế điều hướng dòng tiền, thông tin và quyền lực mà 1% tinh hoa luôn âm thầm vận dụng.")
        for law in ELITE_HIDDEN_LAWS:
            with st.expander(f"{law['icon']} Mật Mã #{law['id']}: {law['name_vi']} ({law['name_en']})"):
                col_l1, col_l2 = st.columns([3, 2])
                with col_l1:
                    st.markdown(f"**⚡ Chân lý gốc (First Principle):**")
                    st.info(law['first_principle'])
                    st.markdown(f"**🚀 Đòn bẩy đè bẹp (Elite Asymmetry):**")
                    st.write(law['elite_asymmetry'])
                    st.markdown(f"**🛠️ Công thức thực chiến cá nhân:**")
                    st.success(law['individual_playbook'])
                with col_l2:
                    st.markdown(f"**⚠️ Cạm bẫy của số đông (Crowd Trap):**")
                    st.error(law['crowd_trap'])
                    st.warning(f"💡 **Ví Dụ Lịch Sử:**  \n{law['real_world_case']}")
    ...
    with m_tab4:
        ...
        if saved_scans:
            s_search = st.text_input(
                ...
                placeholder="Nhập từ khóa (vd: chip, robot, xe điện, năng lượng...)",
                key="room_macro_history_search"
            )
            filtered = saved_scans
            if s_search.strip():
                kw = s_search.strip().lower()
                filtered = [s for s in saved_scans if kw in s.get("query", "").lower() or kw in str(s.get("result", {})).lower()]
...
def render_problem_decomposition_room(active_api_key: str | None = None):
    """Render phòng chức năng Phân Rã Thực Chiến 9 Lăng Kính & Elite Decision Journal."""
    ...
    p_tab1, p_tab2, p_tab3 = st.tabs([
        ...
        "📓 Elite Decision Journal (Đo Sai Số Quyết Định)",
        f"📜 Kho Lưu Trữ Bản Phân Rã ({len(load_problem_analyses())})",
    ])
    ...
    with p_tab1:
        ...
        if "latest_decomposition_result" in st.session_state:
            ...
            if st.button("📓 Chuyển phân rã này thành Bản ghi Quyết định để kiểm chứng sau 30-90 ngày", type="primary"):
                ...
                st.info("👉 Đã nạp dữ liệu vào form! Vui lòng chuyển sang tab **'📓 Elite Decision Journal'** phía trên để lưu!")

    # -------------------------------------------------------------------------
    # TAB 2: ELITE DECISION JOURNAL (NHẬT KÝ QUYẾT ĐỊNH & HIỆU CHUẨN SAI SỐ)
    # -------------------------------------------------------------------------
    with p_tab2:
        st.markdown("#### 📓 Elite Decision Journal: Đo Lường & Hiệu Chuẩn Sai Số Nhận Thức")
        st.caption(
            "Phương pháp của Ray Dalio & Daniel Kahneman: Ghi lại giả định HÔM NAY $\\rightarrow$ Hẹn lịch kiểm định "
            "sau 30/90/180 ngày $\\rightarrow$ Chấm điểm thực tế để đo độ lệch nhận thức (Calibration Score) và diệt trừ "
        ...
        with c_st5:
            st.metric("🎯 Độ Chuẩn Xác (Calibration)", f"{stats['calibration_accuracy']}%")
        ...
        with st.expander("➕ GHI NHẬN QUYẾT ĐỊNH MỚI CẦN KIỂM CHỨNG", expanded=bool(st.session_state.get("dj_prefill_title"))):
            with st.form("form_create_decision"):
                f_title = st.text_input(
                    ...
                    placeholder="Ví dụ: Giải ngân mua gom 30% HPG ở vùng giá 25; Nghỉ việc chuyển sang AI Startup..."
                )
                ...
                f_feynman = st.text_input(
                    "Thử nghiệm trung thực Feynman (Bạn có đang tự lừa dối bản thân về điều gì không?):",
                ...
                with st.expander(expander_label, expanded=(d_status == "due")):
                    ...
                    if d_status in ["due", "pending"]:
                        ...
                        with st.form(f"form_review_{d_id}"):
                            ...
                            col_sub_rev, col_del_d = st.columns([3, 1])
                            with col_sub_rev:
    ...
    # -------------------------------------------------------------------------
    # TAB 3: KHO LƯU TRỮ BẢN PHÂN RÃ (DECOMPOSITION ARCHIVE)
    # -------------------------------------------------------------------------
    with p_tab3:
        st.markdown("#### 📜 Kho Lưu Trữ Bản Phân Rã Vấn Đề")
        ...
        if not all_analyses:
            ...
            filtered_ana = all_analyses
            if s_ana.strip():
                kw_a = s_ana.strip().lower()
                filtered_ana = [a for a in all_analyses if kw_a in a.get("problem", "").lower() or kw_a in str(a.get("full_result", {})).lower()]
            ...
            for a_idx, a_item in enumerate(filtered_ana):
                ...
                with st.expander(f"📌 [{a_time}] {short_prob}"):
                    ...
                    with c_ar:
                        if st.button("🗑️ Xóa bản ghi", key=f"btn_del_ana_{a_id}"):
